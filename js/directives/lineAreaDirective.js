...
define(['app', 'echarts'], function(app, echarts){
   app.directive('lineArea', function(ChartService){
       return {
           scope:{
               id: '=',
               width: '=',
           ...
           link: function($scope, element, attrs){
               ...
               // 设置chart默认data
               var defaultDatas = {
                   name: ['邮件营销','联盟广告'], //对应图表legend
                   xData: ['周一','周二','周三','周四','周五','周六','周日'],
                   yData: [[120, 132, 101, 134, 90, 230, 210],[220, 182, 191, 234, 290, 330, 310]]
               };
               // 设置chart id
                       ...
                       yAxis : [
                           {
                               ...
                               axisLabel:{
                                   formatter: function(value){
                                       return attrs.unit ? value + attrs.unit : value;
                                   }
                               }
                           }
                       ],
               ...
               function setNewDataOption (opt, value){
                   for (var i = 0; i < value.yData.length; i ++){
                       value.yData.map(function(item, index){
                           if (i == index){
                               opt.series[index].data = item;
                           }
                       });
                       opt.series[i].name = value.xData[i];
                   }
